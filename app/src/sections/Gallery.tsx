...
import { useGalleryImages } from '@/features/siteImages/queries';
...
const FALLBACK_TILES: GalleryTile[] = [
  { src: GALLERY_IMAGES[0], caption: 'Cosecha de los Yungas 🌱', span: 'lg:col-span-2 lg:row-span-2' },
  { src: GALLERY_IMAGES[1], caption: 'Tueste fresco de hoy', span: 'lg:col-span-1 lg:row-span-1' },
  { src: GALLERY_IMAGES[2], caption: 'El ritual de las 8am ☕', span: 'lg:col-span-1 lg:row-span-1' },
  { src: GALLERY_IMAGES[3], caption: 'El arte latte perfecto', span: 'lg:col-span-1 lg:row-span-1' },
  { src: GALLERY_IMAGES[4], caption: 'Frío pero con alma', span: 'lg:col-span-1 lg:row-span-2' },
  { src: GALLERY_IMAGES[5], caption: 'Matcha ceremonial, Uji 🇯🇵', span: 'lg:col-span-1 lg:row-span-1' },
  { src: GALLERY_IMAGES[6], caption: 'Caramelo andino', span: 'lg:col-span-1 lg:row-span-1' },
  { src: GALLERY_IMAGES[7], caption: 'Granitos de altura', span: 'lg:col-span-2 lg:row-span-1' },
];
      ...
      <img
        ...
        className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-out group-hover:scale-110${
          src === '/assets/wallaper_2.webp'
            ? ''
            : ' filter grayscale-[30%] group-hover:grayscale-0'
        }`}
      />
...
function FeedCtaTile() {
  return (
    <motion.a
      ...
      transition={{
        duration: 0.65,
        delay: FALLBACK_TILES.length * 0.08,
        ease: EASE,
      }}
...
export default function Gallery() {
  const { data: gallerySrcs } = useGalleryImages();
  const TILES: GalleryTile[] =
    gallerySrcs && gallerySrcs.length >= 8
      ? FALLBACK_TILES.map((t, i) => ({ ...t, src: gallerySrcs[i] ?? t.src }))
      : FALLBACK_TILES;
  return (
    <section
      id="galeria"
      ...
      <div className="relative z-10 container-ocheto">
        ...
        >
          ...
          <p className="mt-5 text-base sm:text-lg text-ocheto-coffee-700/75 max-w-xl mx-auto leading-relaxed">
            Barra, tueste y mesas reales. Sin filtro.
          </p>
