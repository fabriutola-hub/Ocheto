...
export default function ShopGrid() {
  ...
  const baseProducts = useMemo(
    () => (allProducts ?? []).filter((p) => p.category === 'beans' || p.category === 'merch'),
    [allProducts],
  );
      ...
      <div className="relative container-ocheto">
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            >
              <div className="w-16 h-16 rounded-full bg-ocheto-berry-600/10 flex items-center justify-center mb-4">
                <ShoppingBag className="w-7 h-7 text-ocheto-berry-600/70" />
              </div>
