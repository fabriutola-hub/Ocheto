...
export default function CartDrawer() {
  ...
  const { setIsOpen, removeItem, updateQuantity, clearCart } = useCartActions();
  const [mounted, setMounted] = useState(false);
  const emptyBagRef = useRef<HTMLDivElement>(null);
  const animateFloat = useInfiniteAnimation(emptyBagRef);
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between mt-2">
                              ...
                              <span className="font-display font-bold text-[hsl(var(--ocheto-green-700))] tabular-nums">
                                {formatCentsShort(item.price * item.quantity)}
                              </span>
                            </div>
                          </div>
                        </div>
            ...
            {items.length > 0 && (
              ...
              >
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold tabular-nums">{formatCentsShort(totalPrice)}</span>
                </div>
