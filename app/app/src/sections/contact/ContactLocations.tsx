...
const cardVariants: Variants = {
  ...
  show: {
    opacity: 1,
    y: 0,
    transition: { duration: 0.8, ease: HEADER_EASE },
  },
};
...
const hourRowVariants: Variants = {
  ...
  show: (i: number) => ({
    opacity: 1,
    x: 0,
    transition: { duration: 0.45, delay: 0.55 + i * 0.07, ease: HEADER_EASE },
  }),
};
...
function LocationCard({ location, index }: LocationCardProps) {
  ...
  return (
    <motion.article
      ...
      className="group relative flex flex-col overflow-hidden rounded-3xl bg-ocheto-cream-50 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.5)] border border-ocheto-cream-200/60 hover-lift"
    >
      {/* ===== IMAGE HEADER ===== */}
      <div className="relative h-56 sm:h-64 overflow-hidden">
        <motion.img
          ...
          transition={{ duration: 0.9, ease: HEADER_EASE }}
        />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-6">
          ...
          >
            Ocheto
            <br />
            <span className="text-ocheto-gold-500">
              {location.name.replace('Ocheto ', '')}
            </span>
          </h3>
        </div>
      </div>
    ...
    >
      ...
      <div className="relative z-10 container-ocheto">
        ...
        <motion.div
          ...
          whileInView="show"
          viewport={{ once: true, margin: '-50px' }}
          className="mt-12 sm:mt-16 lg:mt-20 grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8"
        >
