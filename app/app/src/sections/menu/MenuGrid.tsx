...
import { getFilteredProducts } from '@/pages/Menu';
import type { MenuCategoryFilter } from '@/pages/Menu';
...
function getCategoryLabel(filter: MenuCategoryFilter): string {
  ...
  if (filter === 'panaderia') return CATEGORIES.panaderia.name.toLowerCase();
  if (filter === 'especialidades') return CATEGORIES.especialidades.name.toLowerCase();
  if (filter === 'sin_cafe') return CATEGORIES.sin_cafe.name.toLowerCase();
  return CATEGORIES[filter].name.toLowerCase();
}

export default function MenuGrid({ activeCategory }: MenuGridProps) {
  const products = getFilteredProducts(activeCategory);
